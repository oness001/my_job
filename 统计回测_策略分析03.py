...
# 打包原回测文件》》训练数据。
def pack_to_traindatas(data_path, train_path, zong_t,time_lidu,hc_zq=6,watch_data=False):
    '''
   1.读取回测文件。
   2.读取训练数据地址，保存一个空文件。
   3.
    ...
    if os.path.exists(train_path):  # 读取保存地址
        ...
        print(f'地址存在。新建一个地址:{train_path}')
        ...
        print(f'路径不存在，新建一个空文件:{train_path}')
    ...
    for k in resdf.keys():  # 遍历里面的策略名
        ...
        #   生成数据
        if len(da.keys()) > 0:
            print(f'{k}数据开始生成训练数据')
            trainda = generate_train_data(da, zong_t=zong_t, hc_zq=6)
...
if __name__ == '__main__':
    ...
    if 0 == True:
        ...
        data_path = r'F:\new_0811\DB_in_out_strategies\MEBD策略模型_03_new.pickle'
        train_path = r'F:\new_0811\huice_log\train_res\MEBD策略模型_03_训练数据.pkl'
        res_path0 = r'F:\new_0811\huice_log\MEBD03_pre_res'
        time_lidu = 1
        zong_t = [dt.datetime(2019, 4, 1), dt.datetime(2020, 6, 1)]
